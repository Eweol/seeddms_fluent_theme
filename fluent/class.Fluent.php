...
class SeedDMS_Theme_Style extends SeedDMS_View_Common {
	...
	function __construct($params, $theme='fluent') {
...
function getOverallStatusIcon($status) { /* {{{ */
	if (is_null($status)) {
		...
		return '<fluent-badge appearance="accent" '.($color ? ' style="color: '.$color.';"' : '').' circular=true>'.getOverallStatusText($status).'</fluent-badge>';
	}
} /* }}} */
...
//		saveState: false,
		...
		openedIcon: $('<img src="<?php echo $this->iconpath ?>chevron_up_20_regular.svg"/>'),
		closedIcon: $('<img src="<?php echo $this->iconpath ?>chevron_down_20_regular.svg"/>'),
/*
...
*/
		...
		onCreateLi: function(node, $li) {
			...
			if(node.is_folder)
				$li.find('.jqtree-title').prepend('<img src="<?php echo $this->iconpath ?>folder_20_regular.svg"/> ').attr('data-name', node.name).attr('rel', 'folder_' + node.id).attr('formtoken', '<?php echo createFormKey(''); ?>').attr('data-uploadformtoken', '<?php echo createFormKey(''); ?>').attr('data-droptarget', 'folder_' + node.id).addClass('droptarget');
			else
				$li.find('.jqtree-title').prepend('<img src="<?php echo $this->iconpath ?>document_20_regular.svg"/> ');
		}
	});
...
<?php
	...
	function __printTreeNavigation($folderid, $showtree){ /* {{{ */
		if ($showtree==1){
			$this->contentHeading("<a href=\"".$this->params['settings']->_httpRoot."out/out.ViewFolder.php?folderid=". $folderid."&showtree=0\"><img src=\"".$this->iconpath."chevron_up_20_regular.svg\"/></a>", true);
...
<?php
			...
			$this->contentHeading("<a href=\"".$this->params['settings']->_httpRoot."out/out.ViewFolder.php?folderid=". $folderid."&showtree=1\"><img src=\"".$this->iconpath."chevron_down_20_regular.svg\"/></a>", true);
		}
	} /* }}} */
...
<?php
	...
	function printDeleteDocumentButton($document, $msg, $return=false){ /* {{{ */
		...
		$content = '';
		$content .= '<a class="delete-document-btn" rel="'.$docid.'" msg="'.getMLText($msg).'" confirmmsg="'.htmlspecialchars(getMLText("confirm_rm_document", array ("documentname" => $document->getName())), ENT_QUOTES).'" title="'.getMLText("delete").'"><img src="'.$this->iconpath.'delete_20_regular.svg"/></a>';
		if($return)
			return $content;
		else
			echo $content;
...
//			$('.delete-document-btn').click(function(ev) {
	...
	function printDeleteFolderButton($folder, $msg, $return=false){ /* {{{ */
		$folderid = $folder->getID();
		$content = '';
		$content .= '<a class="delete-folder-btn" rel="'.$folderid.'" msg="'.getMLText($msg).'" confirmmsg="'.htmlspecialchars(getMLText("confirm_rm_folder", array ("foldername" => $folder->getName())), ENT_QUOTES).'" title="'.getMLText("delete").'"><img src="'.$this->iconpath.'delete_20_regular.svg"/></a>';
		if($return)
			return $content;
		else
			echo $content;
...
//			$('.delete-folder-btn').click(function(ev) {
	...
	function printLockButton($document, $msglock, $msgunlock, $return=false) { /* {{{ */
		...
		$content = '';
		$content .= '<a class="lock-document-btn" rel="'.$docid.'" msg="'.getMLText($msg).'" title="'.getMLText($title).'" data-formtoken="'.createFormKey('tooglelockdocument').'"><img src="'.$this->iconpath.$icon.'_20_regular.svg"/></a>';
		if($return)
			return $content;
		else
			echo $content;
	...
	function printAccessButton($object, $return=false) { /* {{{ */
		...
		if($object->isType('document')) {
			if($accessobject->check_view_access('DocumentAccess'))
				$content .= '<a class="access-document-btn" href="'.$this->params['settings']->_httpRoot.'out/out.DocumentAccess.php?documentid='.$objid.'" title="'.getMLText('edit_document_access').'"><img src="'.$this->iconpath.'flash_20_regular.svg"/></a>';
		} elseif($object->isType('folder')) {
			if($accessobject->check_view_access('FolderAccess'))
				$content .= '<a class="access-folder-btn" href="'.$this->params['settings']->_httpRoot.'out/out.FolderAccess.php?folderid='.$objid.'" title="'.getMLText('edit_folder_access').'"><img src="'.$this->iconpath.'flash_20_regular.svg"/></a>';
		}
		if($return)
			return $content;
		else
...
<?php
	...
	function printDeleteAttributeValueButton($attrdef, $value, $msg, $return=false){ /* {{{ */
		$content = '';
		$content .= '<a class="delete-attribute-value-btn" rel="'.$attrdef->getID().'" msg="'.getMLText($msg).'" attrvalue="'.htmlspecialchars($value, ENT_QUOTES).'" confirmmsg="'.htmlspecialchars(getMLText("confirm_rm_attr_value", array ("attrdefname" => $attrdef->getName())), ENT_QUOTES).'"><img src="'.$this->iconpath.'delete_20_regular.svg"/></a>';
		if($return)
			return $content;
		else
			echo $content;
...
<?php
	...
	public function folderListHeaderImage() { /* {{{ */
		...
		$headcol = ($parent ? '<fluent-button appearance="stealth" id="goto-parent" data-parentid="'.$parent->getID().'"><img src="'.$this->iconpath.'arrow_up_24_regular.svg"/></fluent-button>' : '')."</th>\n";	
		return $headcol;	
	} /* }}} */
	...
	function documentListRowStatus($latestContent) { /* {{{ */
		...
		if ( $document->isLocked() ) {
			...
			$attentionstr .= '<img title="'. getMLText("locked_by").": ".htmlspecialchars($document->getLockingUser()->getFullName()).'" src="'.$this->iconpath.'document_lock_20_regular.svg"/> ';
		}
	...
	function documentListRowAction($document, $previewer, $skipcont=false, $version=0, $extracontent=array()) { /* {{{ */
		...
		if($accessop->check_view_access('RemoveDocument')) {
			if($document->getAccessMode($user) >= M_ALL) {
				...
				$actions['remove_document'] = '<span style="padding: 2px; color: #CCC;"><img src="'.$this->iconpath.'delete_20_regular.svg"/></span>';
			}
		}
		$docID = $document->getID();
		if($document->getAccessMode($user) >= M_READWRITE) {
			$actions['edit_document'] = '<a href="'.$this->params['settings']->_httpRoot.'out/out.EditDocument.php?documentid='.$docID.'" title="'.getMLText("edit_document_props").'"><img src="'.$this->iconpath.'edit_20_regular.svg"/></a>';
		} else {
			$actions['edit_document'] = '<span style="padding: 2px; color: #CCC;"><img src="'.$this->iconpath.'edit_20_regular.svg"/></span>';
		}
		if($document->getAccessMode($user) >= M_READWRITE) {
		...
		if($enableClipboard) {
			$actions['add_to_clipboard'] = '<a class="addtoclipboard" rel="D'.$docID.'" msg="'.getMLText('splash_added_to_clipboard').'" title="'.getMLText("add_to_clipboard").'"><img src="'.$this->iconpath.'copy_20_regular.svg"/></a>';
		}
		if($onepage)
			$actions['view_document'] = '<a href="'.$this->params['settings']->_httpRoot.'out/out.ViewDocument.php?documentid='.$docID.'" title="'.getMLText("view_document").'"><img src="'.$this->iconpath.'eye_20_regular.svg"/></a>';
	...
	function documentListRow($document, $previewer, $skipcont=false, $version=0, $extracontent=array()) { /* {{{ */
		...
		if($latestContent) {
			...
			$content .= "<td class=\"wordbreak\"".($onepage ? ' style="cursor: pointer;"' : '')." vertical-align: middle;\">";
			if($onepage)
			...
			if($categories = $document->getCategories()) {
				...
				foreach($categories as $category)
					...
					$content .= "<fluent-badge appearance=\"accent\" circular=true>".$category->getName()."</fluent-badge> ";
			}
...
//		$folder = $this->params['folder'];
		...
		$content = '';
		$content .= "<div style=\"text-align: center; vertical-align: middle;\" class=\"list-action\">";
		$actions = array();
		if(!empty($extracontent['begin_action_list']))
			$content .= $extracontent['begin_action_list'];
		...
		if ($accessop->check_view_access('RemoveFolder')) {
			if($subFolderAccessMode >= M_ALL) {
				...
				$actions['remove_folder'] = '<span style="padding: 2px; color: #CCC;"><img src="'.$this->iconpath.'delete_20_regular.svg"/></span>';
			}
		}
		if ($accessop->check_view_access('EditFolder')) {
			if($subFolderAccessMode >= M_READWRITE) {
				$actions['edit_folder'] = '<a class_="btn btn-mini" href="'.$this->params['settings']->_httpRoot.'out/out.EditFolder.php?folderid='.$subFolder->getID().'" title="'.getMLText("edit_folder_props").'"><img src="'.$this->iconpath.'edit_20_regular.svg"/></a>';
			} else {
				$actions['edit_folder'] = '<span style="padding: 2px; color: #CCC;"><img src="'.$this->iconpath.'edit_20_regular.svg"/></span>';
			}
		}
		if($subFolderAccessMode >= M_READWRITE) {
		...
		if($enableClipboard) {
			$actions['add_to_clipboard'] = '<a class="addtoclipboard" rel="F'.$subFolder->getID().'" msg="'.getMLText('splash_added_to_clipboard').'" title="'.getMLText("add_to_clipboard").'"><img src="'.$this->iconpath.'copy_20_regular.svg"/></a>';
		}
		if($onepage)
			$actions['view_folder'] = '<a href="'.$this->params['settings']->_httpRoot.'out/out.ViewFolder.php?folderid='.$subFolder->getID().'" title="'.getMLText("view_folder").'"><img src="'.$this->iconpath.'eye_20_regular.svg"/></a>';
...
class_alias('SeedDMS_Theme_Style', 'SeedDMS_Fluent_Style');