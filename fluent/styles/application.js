...
//		console.log($(ev.target).parent().children('[name=missing-lang-translation]').val());
	...
	$('div.ajax').each(function(index) { /* {{{ */
		...
		if(!element.data('no-spinner'))
			element.prepend('<div style="position: _absolute; overflow: hidden; background: #f7f7f7; z-index: 1000; height: 200px; width: '+element.width()+'px; opacity: 0.7; display: table;"><div style="display: table-cell;text-align: center; vertical-align: middle; "><fluent-progress-ring style="display: inline-block;"/></div>');
	...
	$('div.ajax').on('update', function(event, param1, callback) { /* {{{ */
		...
		console.log(url);
		if(!element.data('no-spinner'))
			element.prepend('<div style="position: absolute; overflow: hidden; background: #f7f7f7; z-index: 1000; height: '+element.height()+'px; width: '+element.width()+'px; opacity: 0.7; display: table;"><div style="display: table-cell;text-align: center; vertical-align: middle; "><fluent-progress-ring style="display: inline-block;"/></div>');
		$.get(url, function(data) {
			element.html(data);
			if(callback)
				callback.call();
